...
module TaggedText =
    ...
    let tagModuleBinding name =
        if keywordFunctions.Contains name then
            mkTag TextTag.Keyword name
        else
            mkTag TextTag.ModuleBinding name
...
module Layout =
    ...
    let (@@) layout1 layout2 =
        apply2 (fun l r -> mkNode l r (Broken 0)) layout1 layout2

    let (@@-) layout1 layout2 =
        apply2 (fun l r -> mkNode l r (Broken 1)) layout1 layout2

    let (@@--) layout1 layout2 =
        apply2 (fun l r -> mkNode l r (Broken 2)) layout1 layout2

    let (@@---) layout1 layout2 =
        apply2 (fun l r -> mkNode l r (Broken 3)) layout1 layout2

    let (@@----) layout1 layout2 =
        apply2 (fun l r -> mkNode l r (Broken 4)) layout1 layout2
    ...
    let commaListL layouts =
        tagListL (fun prefixL -> prefixL ^^ rightL comma) layouts

    let semiListL layouts =
        tagListL (fun prefixL -> prefixL ^^ rightL semicolon) layouts

    let spaceListL layouts = tagListL id layouts

    let sepListL layout1 layouts =
        tagListL (fun prefixL -> prefixL ^^ layout1) layouts

    let bracketL layout =
        leftL leftParen ^^ layout ^^ rightL rightParen

    let tupleL layouts =
        bracketL (sepListL (sepL comma) layouts)
    ...
    let boundedUnfoldL (itemL: 'a -> Layout) (project: 'z -> ('a * 'z) option) (stopShort: 'z -> bool) (z: 'z) maxLength =
            ...
            if stopShort z then
                ...
                | Some(x, z) ->
                    if n <= 0 then
                        ...
                        itemL x :: consume (n - 1) z // cons recursive...
        ...
        let GetValueInfoOfObject (bindingFlags: BindingFlags) (obj: obj) =
            ...
            | _ ->
                ...
                // true structure of the type, e.g. the order of the fields.
                    ...
                    let pvals =
                        (props, vals) ||> Array.map2 (fun prop v -> prop.Name, (v, prop.PropertyType))
...
module Display =
    ...
    let getField obj (fieldInfo: FieldInfo) = fieldInfo.GetValue(obj)
    ...
    let formatString (s: string) =
        let rec check i =
            i < s.Length && not (Char.IsControl(s, i)) && s[i] <> '\"' && check (i + 1)

        let rec conv i acc =
            if i = s.Length then
                combine (List.rev acc)
            else
                conv (i + 1) (formatChar false s[i] :: acc)
    ...
    type ObjectGraphFormatter(opts: FormatOptions, bindingFlags) =
        ...
        and mapSetValueL depthLim prec (ty: Type) (obj: obj) =
            let word =
                if ty.GetGenericTypeDefinition() = typedefof<Map<int, int>> then
                    "map"
                else
                    "set"
